...
export function getMyQueue(bookingId, dateText, resId, timeText) {
  return (dispatch) => {
    ...
    FirebaseService.database().ref().child('bookings').child('online')
      .orderByChild('status_dateText_resId').equalTo('booking_' + dateText + '_' + resId).on('value', (snap) => {
        const count = countQueue(snap, bookingId, timeText);
        dispatch(getMyQueueSuccess(count))
      })
  }
}

function countQueue(snap, id, time) {
  var count = 1;
  var target = 1;
  snap.forEach(booking => {
    if (booking.val().timeText == time) {
